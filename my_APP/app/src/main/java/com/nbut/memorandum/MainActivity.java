...
//
//        RecycleBinAdapter adapter = new RecycleBinAdapter(deletedNotes); //上面声明    private List<Note> deletedNotes;
...
//        setContentView(R.layout.activity_main);
//
//        // 确保 deletedNotes 已初始化
//        if (deletedNotes == null) {
//            deletedNotes = new ArrayList<>();  // 或者加载数据
//        }
//
//
//        // 将recyclerView声明为final
//        final RecyclerView recyclerView = findViewById(R.id.recycler_view);  // 在onCreate中初始化
//
//        // 设置布局管理器
//        StaggeredGridLayoutManager staggeredGridLayoutManager =
//                new StaggeredGridLayoutManager(2, StaggeredGridLayoutManager.VERTICAL);
...
//
//        // 设置适配器
//        RecycleBinAdapter adapter = new RecycleBinAdapter(deletedNotes); // deletedNotes 需要提前初始化
//        recyclerView.setAdapter(adapter);
//
//        // 设置Toolbar
//        main_toolbar = findViewById(R.id.main_toolbar);
...
//            @Override
//            public void onTabSelected(TabLayout.Tab tab) {
//                String flag = (String) tab.getText();
//                if (!flag.equals("首页")) {
//                    useAdapter(recyclerView, loadAppointData(flag), flag);
//                } else {
//                    useAdapter(recyclerView, noteList, flag);
...
//
//        // 将便签添加到回收站
//        recycleBinDatabase.addDeletedNote(note);
//
//        // 更新 RecyclerView
...
//        return super.onOptionsItemSelected(item);
        switch (item.getItemId()) {
            ...
            case R.id.recycleBin:
                Intent toRecycleBin = new Intent(MainActivity.this, RecycleBinActivity.class);
                startActivity(toRecycleBin);
                break;
            default:
        }
        return true;
...
//                                    uriTabs,
//                                    new ContentValues() {{ put("isDeleted", 1); }},
//                                    "tabName = ?",
...
//                                    new ContentValues() {{ put("isDeleted", 1); }},
//                                    "flag = ?",
//                                    new String[]{tabsList.get(i).getTabName()}
//                            );
    ...
    private void useAdapter(RecyclerView recyclerView, final List<Note> currentNoteList, final String flag) {
        ...
        adapter.setOnItemClickListener(new NoteAdapter.ItemClickListener() {
            ...
            public void onItemClick(int position) {
                ...
                // 根据 ID 获取备忘录
                Note record = getNoteWithNum(cuId);
                // 将选中的备忘录数据传递给 Edit Activity
                transportInformationToEdit(intent, record);
                startActivityForResult(intent, position);  // 跳转到编辑页面
            }
        });
